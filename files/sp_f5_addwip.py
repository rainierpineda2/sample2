...
# Author: anthony.pineda@spglobal.com
# Name: sp_f5_addwip
...
def check_srv_exist(obj, vs_srv):
    try:
       srvlist = obj.GlobalLB.Server.get_list()
    except Exception, e:
        print e
    for srv in srvlist:
        if srv == vs_srv:
...
def create_gtm_srv(obj, vs_srv, vs_addr, vs_dc):
    try:
       obj.GlobalLB.Server.create_v2(servers = [vs_srv], ips = [[{'device': vs_addr, 'addresses_on_device': [vs_addr]}]], \
           types = ['SERVER_TYPE_GENERIC_HOST'], data_centers = [vs_dc])
    except Exception, e:
       z = ''.join(e)
...
# Create a list of virtual server order; check if virtual server discovery is disabled on server
        ...
        for m in server:
            ...
            vsdc = m['dc']
            if vssrv != prevsrv:
                create_gtm_srv(b, vssrv, vsaddr, vsdc)
                prevsrv = vssrv
